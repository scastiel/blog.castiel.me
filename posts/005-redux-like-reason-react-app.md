...
Please know that il will be easier to understand the tutorial if you already have some knowledge about Reason syntax, about Redux-like architecture (we'll keep it very simple here), and maybe also about React.
...
So as we won't be able to write something like `const state = { counter: 0, intervalId: 123 }`, we'll write `const state = setCounter(createState(), 0)`.
